...
class ViT02(nn.Module):
    ...
    def forward(self, x):
        residual = x
        x = F.interpolate(x.float(), size=(1280, 1280), mode='nearest')
        ...
        residual = x
        x = F.interpolate(x.float(), size=(37, 37), mode='bilinear')
        x = x.to(dtype=residual.dtype)
        B, C, H, W = x.shape
...
def build_codetr_vit_l(checkpoint=None):
    return _build_codetr(
