...
if __name__ == "__main__":
    ...
    GT = get_gt(
        data_path=os.path.join(args.data_dir, 'images')
    )

    experiment = args.experiment

    result_convert_dir = os.path.join(args.result_dir, 'answers_convert', experiment)
    os.makedirs(result_convert_dir, exist_ok=True)

    answers = [json.loads(line) for line in open(os.path.join(args.result_dir, 'answers', f'{experiment}.jsonl'))]
    ...
    for category, cate_tups in results.items():
        with open(os.path.join(result_convert_dir, f'{category}.txt'), 'w') as fp:
